...
10. **Switching to StaticLiveServerTestCase**。修改functional_tests/tests.py文件中修改`from django.contrib.staticfiles.testing importStaticLiveServerTestCase`和`class NewVisitorTest(StaticLiveServerTestCase):`。运行功能测试`python manage.py test functional_tests`，会提示Ran 3 tests in 21.068s和OK。
...
12. **Using Our Own CSS**，base.html中添加一行`<link href="/static/base.css" rel="stylesheet">`，在lists/static下新建base.css文件，内容为`#id_new_item {    margin-top: 2ex;}`。浏览器打开`http://localhost:8000/`可以看到整体会有2像素（？）的偏移。运行功能测试`python manage.py test functional_tests`，会提示Ran 3 tests in 21.130s和OK。
