...
6.  查看test.py代码，修改test_home_page_returns_correct_html函数如下。
```python
...
7. 再把以上函数改成下面上方的代码。②和③一样。删掉②后如下面下方所示，同时修改函数名为test_uses_home_template。运行`python manage.py test`，提示成功。
```python
def test_home_page_returns_correct_html(self):
...
11. functional_tests3.py中添加`"New to-do item did not appear in table"`后如下，并`python functional_tests3.py`，会提示AssertionError: False is not true : New to-do item did not appear in table。
```python
self.assertTrue(
...
6. **On Implicit and Explicit Waits, and Voodoo time.sleeps。**functional_tests下的tests.py中添加如下代码，同时删掉`check_for_row_in_list_table`函数，添加如下的 `wait_for_row_in_list_table`函数。删掉`test_can_start_a_list_and_retrieve_it_later`函数中的`time.sleep(1)`代码，同时把调用前者（？）函数的部分改成调用`wait_for_row_in_list_table`函数。运行`python manage.py test`后会提示AssertionError: Finish the test!和Ran 7 tests in 5.250s，时间变短了。
```python
...
7. 导入自己的或者老师的github代码，`git clone https://github.com/zhang99969/suplerlists.git \ ~/sites/$SITENAME/source` 或者
`git clone https://github.com/yuanyuyu/-TDD.git \ ~/sites/$SITENAME/source`，此处拉老师的代码。提示如下，此时在**/home/zhang/sites/39.96.9.216/**目录下有database、source、static和virtualenv文件夹。source文件夹内为拉取的代码。空格加\代表换行。

    ```shell
    $ git clone https://github.com/yuanyuyu/-TDD.git \ ~/sites/$SITENAME/source> 
...
10. 本地Anaconda Prompt内输入`pip install fabric3`进行安装。在本地`21 _Software Testing Technology\suplerlists3\deploy_tools`下新建fabfile.py文件，内容见https://github.com/yuanyuyu/-TDD/tree/master/deploy_tools 。注意在本地代码的fabfile.py中，注释掉了`run(f'cd {source_folder} && git reset --hard {current_commit}')，因为在Windows环境下运行时一直有错误。
...
14. 教材中部署了两个域名包括staging服务器和living服务器，故需要再此设置为两个不同的用户名。例如老师在yuyuyuan用户名的基础上又建立过了yuyu用户名，在部署到live时则输入`fab deploy:host=yuyu@39.96.9.216`。在自己这边则是zhang用户名和root用户名，或者zht用户名。

15. 因此部署完之后输入以下代码时需要有改动，因为之前已经在之前的命令创建过39.96.9.216的文件夹，因此此时为新建39.96.9.216_2的文件夹。**注意**/etc/systemd/system路径下的gunicorn-39.96.9.216.service和gunicorn-zht（第二个用户名）.service两个文件内的用户名要各自对应相同，同时deploy_tools目录下各自的gunicorn-systemd.template.service和nginx.template.conf也可以相应的用户名对应，不改也没关系。**接着**输入下方四条命令来运行服务器。
```shell
sed "s/SITENAME/39.96.9.216/g" source/deploy_tools/nginx.template.conf | sudo tee /etc/nginx/sites-available/39.96.9.216_2

sudo ln -s ../sites-available/39.96.9.216_2 /etc/nginx/sites-enabled/39.96.9.216_2

sed "s/SITENAME/39.96.9.216/g" source/deploy_tools/gunicorn-systemd.template.service | sudo tee /etc/systemd/system/gunicorn-zht(第二个用户名).service
```
...
16. 修改其中一个39.96.9.216/source/lists/template下的home.html中的一些内容，使两个用户名下的home.html页面不一样。切换时需要输入以下两条命令来关掉一个，接着开启另一个。在浏览器打开http://39.96.9.216/ 后刷新页面可以看到页面内的内容有变化。张和辉说主要在于第一个的nginx.template.conf。
```shell
sudo systemctl stop gunicorn-39.96.9.216
sudo systemctl restart gurincorn-zht
```
