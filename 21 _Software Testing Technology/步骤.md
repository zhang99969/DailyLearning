...
18. **Biting the Bullet: Adjusting Our Models**。修改lists/tests.py的class ItemModelTest(TestCase)为ListAndItemModelsTest(TestCase)，并在其中添加多行代码，见7.pdf的114页。运行单元测试`manage.py test lists`，首先会提示ImportError: cannot import name 'List'，在models.py中添加如下代码，运行单元测试manage.py test lists后

class List(Object):
    pass后AttributeError: 'List' object has no attribute 'save'
    
    class List(models.Model):
    pass后会提示django.db.utils.OperationalError: no such table: lists_list

进行数据库迁移python manage.py makemigrations。
...
后python manage.py test lists。
AssertionError: 'List object (1)' != <List: List object (1)>

改成list = models.ForeignKey(List, default=None)。
接着改成    list = models.ForeignKey(List, on_delete=models.CASCADE,default=None)
去掉迁移rm suplerlists\lists\0003_auto_20190430_1908.py，直接在文件管理器删除。

后再迁移
Migrations for 'lists':
  lists\migrations\0003_auto_20190430_1913.py
    - Create model List
    - Add field list to item
运行单元测试`manage.py test lists`，上面会有sqlite3.IntegrityError: NOT NULL constraint failed: lists_item.list_id提示和下面有FAILED (errors=3)提示。最后models中代码如下。
   ```python
class List(models.Model):
    pass
class Item(models.Model):
...
19. 在views.py中添加如下内容。运行单元测试，Ran 6 tests in 0.044s成功。
...
20. 运行功能测试python manage.py test functional_tests，会提示AssertionError: '1: Buy milk' not found in ['1: Buy peacock feathers', '2: Buy milk']错误。
...
21. **Each List Should Have Its Own URL**。修改lists/tests.py中的函数如下，见7.pdf的120页。运行单元测试`manage.py test lists`会提示AssertionError: 404 != 200 : Couldn't retrieve content: Response code was 404 (expected 200)。和AssertionError: No templates used to render the response。

22. 把urls.py中部分代码修改为如下，运行单元测试`manage.py test lists`，会提示TypeError: view_list() takes 1 positional argument but 2 were given。views.py中def view_list(request)改成def view_list(request,list_id)后运行单元测试`manage.py test lists`，AssertionError: 1 != 0 : Response should not contain 'other list item 1'。接着把views.py中def view_list(request, list_id)函数改成下方代码，运行单元测试会提示ValueError: invalid literal for int() with base 10: 'the-only-list-in-the-world'。
   ```python
url(r'^$', views.home_page, name='home'),
url(r'^lists/new$', views.new_list, name='new_list'),
...
23. **Adjusting new_list to the New World**。lists/tests.py中def test_redirects_after_POST(self):改成
   ```python
   response = self.client.post('/lists/new', data={'item_text': 'A new list item'})
new_list = List.objects.first()
self.assertRedirects(response, f'/lists/{new_list.id}/')
   ```
 单元测试后提示ValueError: invalid literal for int() with base 10: 'the-only-list-in-the-world'。
views.py中def new_list(request)函数改成
   ```python
    list_ = List.objects.create()
    Item.objects.create(text=request.POST['item_text'], list=list_)
...
运行单元测试，提示Ran 6 tests in 0.054s和OK。
    
23. **The Functional Tests Detect Another Regression**。运行功能测试，python manage.py test functional_tests，提示
    AssertionError: '2: Use peacock feathers to make a fly' not found in ['1: Use peacock feathers to make a fly']

----------------------------------------------------------------------
Ran 2 tests in 25.364s

24. **One More View to Handle Adding Items to an Existing List**。在lists/tests.py添加7.pdf的12页的内容。运行单元测试，提示AssertionError: 301 != 302 : Response didn't redirect as expected: Response code was 301 (expected 302)。

25. **Beware of Greedy Regular Expressions!**。urls.py中一行改成`url(r'^lists/(\d+)/$', views.view_list, name='view_list'),`

还是AssertionError: 404 != 302 : Response didn't redirect as expected: Response code was 404 (expected 302)

26.**The Last New URL**。修改urls.py为如下
    url(r'^$', views.home_page, name='home'),
    url(r'^lists/new$', views.new_list, name='new_list'),
...
27. **The Last New View**。在views.py中添加代码直到改成7.pdf的126页。直到如下。
def add_item(request, list_id):
list_ = List.objects.get(id=list_id)
Item.objects.create(text=request.POST['item_text'], list=list_)
return redirect(f'/lists/{list_.id}/')
运行单元测试，Ran 8 tests in 0.047s和OK。

28.**Testing the Response Context Objects Directly**。在ists/templates/list.html中修改一行为`<form method="POST" action="/lists/{{ list.id }}/add_item">`<form method="POST" action="/lists/{{ list.id }}/add_item">。在lists/views.py添加新的单元测试7.pdf的127页。添加到？的class中。
...
单元测试，KeyError: 'list'。
views.py中def view_list(request,list_id):简化，
def view_list(request, list_id):
list_ = List.objects.get(id=list_id)
return render(request, 'list.html', {'list': list_})
。单元测试，AssertionError: False is not true : Couldn't find 'itemey 1' in response。
lists.html中修改一行为            {% for item in list.item_set.all %}
                <tr><td>{{ forloop.counter }}: {{ item.text }}</td></tr>
            {% endfor %}
       
       单元测试Ran 9 tests in 0.062s

OK
功能测试Ran 2 tests in 15.338s

OK

29. **A Final Refactor Using URL includes**。uperlists/urls.py中修改代码
