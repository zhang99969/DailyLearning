...
1. **Ensuring We Have a Regression Test。**functional_tests下的tests.py中test_can_start_a_list_and_retrieve_it_later函数更名为test_can_start_a_list_for_one_user，并修改和删除部分如下，添加见7.pdf的99和100页的函数test_multiple_users_can_start_lists_at_different_urls。运行`python manage.py test functional_tests`后会提示Regex didn't match: '/lists/.+' not found in 'http://localhost:14533/'和Ran 2 tests in 10.339s和FAILED (failures=1)。
 ```python
