...
2. **Spinning Up a Server**。打开xshell连接到阿里云服务器， 依次输入以下命令，并在必要时选择Y或者install the package maintainer''s version。完成后在浏览器中输入服务器实例的公网IP，如http://39.96.9.216/ 。可以看到如下下方内容。
```shell
    apt-get update
    apt-get upgrade
...
```
nginx可能会遇到问题，要删掉configure里面一个东西、
```
>   Welcome to nginx!
...
4. 申请一个非root用户。useradd zhang，passwd zhang 后输入密码123，
需使用root账户重新创建该用户home目录，以账户zhang为例。
...
6. 使用新建的用户名zhang（下同）在服务器在/home/zhang/目录下依次输入以下命令。
```shell
...
~/sites/$SITENAME/source`
，此处拉老师的代码。提示如下，此时在**/home/zhang/sites/39.96.9.216/**目录下有database、source、static和virtualenv文件夹。source文件夹内为拉取的代码。
...
11. 在服务器远程连接终端中输入`../virtualenv/bin/python manage.py runserver`提示一下内容。在虚拟环境下启动了服务器。
...
13. 依次在服务器远程连接终端中输入以下命令。首先查看局部变量是否还存在，接着创建一个快捷方式。最后一条命令会提示下方中间内容。接着输入下方下面的命令，来删掉default。
```shell
...
>使用**../virtualenv/bin/python manage.py runserver**启动服务器。也可以用nginx start开 sudo systemctl start nginx  **?**

17. 在本地连接服务器进行功能测试，使用`python manage.py test functional_tests STAGING_SERVER=39.96.9.216`命令，还不行。提示STAGING_SERVER=39。

改变本地代码（如修改为self.live_server_url='http://39.96.9.216' ），直接打开网页来进行连接服务器的功能测试

Ran 3 tests in 21.539s和OK和Destroying test database for alias 'default'...
...
会提示No module named 'suplerlists'错误，找到/home/zhang/sites/39.96.9.216/source/superlists下的wsgi.py，修改一行为
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "superlists.settings")。
重新../virtualenv/bin/gunicorn superlists.wsgi:application
打开浏览器http://39.96.9.216/ 可以看到css格式消失了。
